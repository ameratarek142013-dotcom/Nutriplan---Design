...
// --- Public: Add Item to Log ---
export function logMeal(meal) {
    
    const existing = loggedItems.find((i) => i.id === meal.id);
    if (existing) {
        existing.servings = (existing.servings || 1) + 1;
        existing.calories = Math.round((meal.calories / (meal.servings || 1)) * existing.servings);
        existing.protein = Math.round((meal.protein / (meal.servings || 1)) * existing.servings);
        existing.carbs = Math.round((meal.carbs / (meal.servings || 1)) * existing.servings);
        existing.fat = Math.round((meal.fat / (meal.servings || 1)) * existing.servings);
    } else {
        loggedItems.push({ ...meal, servings: 1, loggedAt: new Date().toISOString() });
    }
    saveToStorage();
    renderFoodLog();
}
...
function renderSummary() {
 ...
 if (pct >= 100) {
            bar.className = "macro-bar bg-red-500 h-2 rounded-full transition-all duration-300";
        } else {
            bar.className = `macro-bar bg-${colors[i]}-500 h-2 rounded-full transition-all duration-300`;
        }
...
function renderLoggedItems() {
    ...
    container.innerHTML = loggedItems
        .map(
            (item) => `
    <div class="flex items-center justify-between gap-4 p-5 bg-gray-50/40 hover:bg-gray-50 rounded-xl border border-gray-100 transition-all" data-item-id="${item.id}">
      
      <div class="flex items-center gap-3 flex-1 min-w-0">
        ${item.image
                    ? `<img src="${item.image}" alt="${item.name}" class="w-20 h-18 rounded-2xl object-cover shrink-0 border border-gray-100" />`
        ...
        <div class="min-w-0">
          ...
          <div class="flex items-center flex-wrap gap-x-2 text-[11px] text-gray-400 mt-0.5">
            ...
            <span style="padding-inline: 5px;" class="text-emerald-600 ">Recipe</span>
            ${item.loggedAt ? `<span class="block w-full" >${new Date(item.loggedAt).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>` : ''}
          </div>
        </div>
      </div>
      ...
      <div class="flex items-center gap-4 shrink-0">
        ...
        <div class="flex items-center gap-1.5 text-[11px] font-medium text-gray-500">
          <span style="margin: 5px; background-color: #EFF6FF;" class="px-2 py-1 bg-white rounded border border-gray-100 text-gray-500 text-sm">${item.protein}g <span class="text-gray-400 font-normal">P</span></span>
          <span style="margin: 5px; background-color: #FFFBEB;" class="px-2 py-1 bg-white rounded border border-gray-100 text-gray-500 text-sm">${item.carbs}g <span class="text-gray-400 font-normal">C</span></span>
          <span style="margin: 5px; background-color: #FAF5FF;" class="px-2 py-1 bg-white rounded border border-gray-100 text-gray-500 text-sm">${item.fat}g <span class="text-gray-400 font-normal">F</span></span>
        </div>

        <button
          class="remove-item-btn w-8 h-8  flex items-center justify-center text-gray-400  hover:text-red-500 transition-colors"
          data-id="${item.id}"
          title="Remove"
        >
...
function removeItem(id) {
    loggedItems = loggedItems.filter((index) => index.id !== id);
    saveToStorage();
    renderFoodLog();
...
function renderWeeklyChart() {
    ...
    for (let i = 6; i >= 0; i--) {
        ...
        const dayCalories = items.reduce((sum, item) => sum + (item.calories || 0), 0);
        ...
        const kcalClass = isToday && dayCalories > 0
            ? "text-emerald-600 font-bold"
            : dayCalories > 0 ? "text-emerald-500 font-semibold" : "text-gray-300 font-medium";
...
function initQuickActions() {
    const [logMealBtn, scanBtn, customBtn] = document.querySelectorAll(".quick-log-btn");
    ...
    Swal.fire({
      ...
      <div class="text-left space-y-3">
        ...
        preConfirm: () => {
            ...
            const calories = parseFloat(document.getElementById("swal-calories").value) || 0;
            const protein = parseFloat(document.getElementById("swal-protein").value) || 0;
            const carbs = parseFloat(document.getElementById("swal-carbs").value) || 0;
            ...
            return { id: `custom_${Date.now()}`, name, calories, protein, carbs, fat, image: null };
        },
    }).then((result) => {
