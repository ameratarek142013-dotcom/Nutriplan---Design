import { recipsGrid } from "./getmeal.js"

export const colors = ["yellow", "pink","orange" , "gray" , "amber" , "red" , "rose" , "blue" , "teal" , "green" , "emerald" , "cyan" , "teal" , "pink"]
const icons = ["drumstick-bite" ,"drumstick-bite" , "cake-candles" , "drumstick-bite" , "bowl-rice", "bowl-food", "bacon" , "fish-fins" ,"plate-wheat" ,"utensils" , "leaf" , "seedling" ]
...
function displayCategory(allCategories){
  ...
  document.getElementById("categories-grid").innerHTML = allCategories.map((category , index)=> {
...
export async function filter(type, value) {
  try {
    ...
    `
    let response = await fetch(`https://nutriplan-api.vercel.app/api/meals/filter?${type}=${value}&page=1&limit=25`);
